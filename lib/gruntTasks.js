 ...
 * @param callback
 */
GruntTasks.prototype.handlePIDFile = function(service, serviceName, callback) {
    var pidFile;
    ...
    if(fs.existsSync(path.join(process.cwd(), '.pid.json'))) {
        ...
        if(pidFile[service].pid != process.pid)
            try {
                process.kill(pidFile[service].pid);
                console.log('Killing previous `pidFile.%s.pid`: %s success', service, pidFile[service].pid);
            }
            catch(e) {
                console.log('Problems killing previous `pidFile.%s.pid`: %s', service, pidFile[service].pid);
            }
    }
    else {
    ...
    console.log("Preparing to start '%s'", serviceName);
    ...
    callback();
    console.log("'%s' started on process pid %s", serviceName, process.pid);
